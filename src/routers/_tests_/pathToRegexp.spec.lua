-- upstream: https://github.com/pillarjs/path-to-regexp/blob/feddb3d3391d843f21ea9cde195f066149dba0be/src/index.spec.ts
return function()
	...
	-- Roblox deviation: this function is used by one test case, so it's hard-coded
	-- to work exactly for that specific case for simplicity
	local function encodeURI(str)
		if str:sub(1, 1) == "/" then
			return "/" .. encodeURIComponent(str:sub(2))
		end
	...
	local TESTS = {
		...
		},
		{
			...
			},
			{
				...
				-- 	"/caf%C3%A9",
				-- 	{"/caf%C3%A9", "caf%C3%A9"},
				-- 	{ path = "/caf%C3%A9", index = 1, params = { test = "café" } },
		...
		-- Roblox deviation: don't support Regexp objects, no need for them currently
		...
		-- Roblox deviation: this failure probably comes from the Regex library implementation
		-- where non-matching may not be implemented
		-- {
		-- 	"(?:.*)",
		-- 	undefined,
		-- 	{},
	...
	-- Roblox deviation: Regex library does not seem to handle named groups very well
	-- and the feature seems optional so
	--[[
	 * Named capturing groups (available from Node version 10)
	 ]]
	-- if gte(process.version, '10.0.0') then
	-- 	TESTS.push({
	-- 		'/(?<groupname>.+)',
	-- 		undefined,
	-- 		{
	...
	describe("path-to-regexp", function()
		...
		describe("arguments", function()
			...
			it("should throw on non-capturing pattern", function()
				...
				end).toThrow(TypeError('Pattern cannot start with "?" at 7'))
			end)

			it("should throw on nested capturing group", function()
				jestExpect(function()
					pathToRegexp.pathToRegexp("/:foo(\\d+(\\.\\d+)?)")
				end).toThrow(TypeError("Capturing groups are not allowed at 10"))
			end)

			it("should throw on unbalanced pattern", function()
				jestExpect(function()
					pathToRegexp.pathToRegexp("/:foo(abc")
				end).toThrow(TypeError("Unbalanced pattern at 6"))
			end)

			it("should throw on missing pattern", function()
				jestExpect(function()
					pathToRegexp.pathToRegexp("/:foo()")
				end).toThrow(TypeError("Missing pattern at 6"))
			end)

			it("should throw on missing name", function()
				jestExpect(function()
					pathToRegexp.pathToRegexp("/:(test)")
				end).toThrow(TypeError("Missing parameter name at 2"))
			end)

			it("should throw on nested groups", function()
				jestExpect(function()
					pathToRegexp.pathToRegexp("/{a{b:foo}}")
				end).toThrow(TypeError("Unexpected OPEN at 4, expected CLOSE"))
			end)

			it("should throw on misplaced modifier", function()
				jestExpect(function()
					pathToRegexp.pathToRegexp("/foo?")
				end).toThrow(TypeError("Unexpected MODIFIER at 5, expected END"))
			end)
		end)

		describe("tokens", function()
			local tokens = pathToRegexp.parse(TEST_PATH)
		...
		describe("rules", function()
			for _, test in ipairs(TESTS) do
				local path, opts, tokens, matchCases, compileCases = unpack(test)
				if opts == null or opts == undefined then
					opts = nil
				end
				...
				describe(util.inspect(path), function()
					...
					if type(path) == "string" then
						...
						describe("compile", function()
							for _, io in ipairs(compileCases) do
								local params, result, options = unpack(io)
								if params == null or params == undefined then
									params = nil
								end
								...
								if result ~= nil and result ~= null then
									it("should compile using " .. util.inspect(params), function()
										jestExpect(toPath(params)).toEqual(result)
									end)
								else
					...
					describe("match" .. (opts and " using " .. util.inspect(opts) or ""), function()
						for _, io in ipairs(matchCases) do
							local pathname, matches, params, options = unpack(io)
							if matches == null or matches == undefined then
								matches = nil
							end
							local message = ("should %smatch %s"):format(
								matches and "" or "not ",
								util.inspect(pathname)
							)
							if matches ~= nil then
								for i = 1, #matches do
									local match = matches[i]
									if match == null or match == undefined then
										matches[i] = nil
									end
								end
							end
							...
							if type(path) == "string" and params ~= nil and params ~= undefined then
								...
								local match = pathToRegexp.match(path, options)

								it(message .. " params", function()
									jestExpect(match(pathname)).toEqual(params)
								end)
							end
						end
					end)
		...
		describe("compile errors", function()
			it("should throw when a required param is undefined", function()
				local toPath = pathToRegexp.compile("/a/:b/c")

				jestExpect(function()
					toPath()
				end).toThrow(TypeError('Expected "b" to be a string'))
			end)
			...
			it("should throw when expecting a repeated value", function()
				local toPath = pathToRegexp.compile("/:foo+")

				jestExpect(function()
					toPath({ foo = {} })
				end).toThrow(TypeError('Expected "foo" to not be empty'))
			end)
			...
			it("should throw when not expecting a repeated value", function()
				...
				jestExpect(function()
					toPath({ foo = {} })
				end).toThrow(TypeError('Expected "foo" to not repeat, but got an array'))
			end)
